...
\begin{itemize}
    ...
    \item We develop block-query tiled Triton kernels with key-centric backward passes achieving 58x forward speedup at 524K tokens (Section \ref{sec:implementation}).
    \item We demonstrate 3.64x end-to-end training speedup at 131K sequence length (Section \ref{sec:experiments}).
...
\midrule
2,048 & 0.11 & 0.09 & 1.24x \\
4,096 & 0.29 & 0.10 & 2.80x \\
8,192 & 0.92 & 0.22 & 4.25x \\
16,384 & 3.37 & 0.34 & 9.77x \\
32,768 & 12.8 & 1.15 & 11.2x \\
65,536 & 48.5 & 2.33 & 20.9x \\
131,072 & 192 & 7.36 & 26.2x \\
262,144 & 765 & 17.3 & 44.3x \\
524,288 & 3054 & 52.4 & \textbf{58.2x} \\
\bottomrule
\end{tabular}
\label{tab:kernel}
\end{table}

The subquadratic scaling is evident: at 524K tokens, Cron Root Attention is 58x faster than SDPA (which uses FlashAttention-2 as its backend), demonstrating the $O(N\sqrt{N})$ vs $O(N^2)$ complexity difference. The crossover point where Cron Root becomes faster is approximately 2K tokens for forward passes and 12K tokens for full training (forward + backward).
...
32K & 24.5 & 43.1 & 1.76x \\
64K & 65.9 & 170 & 2.58x \\
128K & 186 & 676 & \textbf{3.64x} \\
\bottomrule
\end{tabular}
\label{tab:training}
\end{table}

Training speedup increases with sequence length due to the quadratic vs subquadratic scaling gap. At 131K tokens, we achieve 3.64x end-to-end speedup. Note that the backward pass has higher constant overhead due to four separate kernels (dQ, local dK/dV, strided dK/dV, relay dK/dV), which explains the higher crossover point (~12K) compared to forward-only (~2K). Attention is approximately 30-40\% of total training compute; the remaining FFN, LayerNorm, and embedding operations limit the theoretical maximum speedup per Amdahl's Law.
